...
Фоновые агенты не должны полагаться на inline image/video placeholders в своих
ответах. Пользователь смотрит визуальные результаты через основной чат с
`FP Lead agent`.

Правило для всех задач с картинками, видео, Figma previews, renders,
comparison sheets:

- агент сохраняет визуальный результат реальным файлом в shared workspace;
- файл должен лежать внутри папки задачи, а не только во временной директории;
- агент возвращает абсолютный Windows-путь к файлу;
- агент проверяет, что файл существует и не пустой;
- `FP Lead agent` открывает файл через `view_image` или другой доступный viewer
  и показывает preview пользователю в основном чате;
- фоновые агенты не отправляют промежуточные скрины/рендеры как image
  attachments, если их нельзя открыть в основном чате;
- для промежуточного review агент пишет текстом `Preview saved:` и абсолютный
  путь к файлу;
- markdown-картинки/видео от фонового агента считаются вспомогательными, но не
  финальным handoff.
